...
def _naive_varying_quantile_groupby(
    ...
    """Naive Python implementation for verification."""
    # Group and collect values into lists, get first quantile value per group
    ...
    for row in grouped.iter_rows(named=True):
        ...
        # Compute quantile using polars Series method
...
def test_quantile_varying_by_group_basic() -> None:
    """Test basic varying quantile functionality."""
    # Test case from issue #20951
    df = pl.DataFrame(
        {
            "value": [1, 2, 1, 2],
            "quantile": [0, 0, 1, 1],
    ...
    expected = pl.DataFrame({"quantile": [0, 1], "value": [1.0, 2.0]})
    assert_frame_equal(result, expected)

    # Verify against naive implementation
    naive_result = _naive_varying_quantile_groupby(df, "quantile", "value", "quantile")
    assert_frame_equal(result, naive_result)
...
def test_quantile_varying_by_group_parametrized(
    method: QuantileMethod, dtype: pl.DataType, test_values: list[float]
) -> None:
    """Parametrized test for varying quantiles across different methods and dtypes."""
    df = pl.DataFrame(
        {
    ...
    # Verify against naive implementation (dtypes may differ)
...
@pytest.mark.parametrize(
    ("dtype", "test_values", "expected_values"),
    [
        (
            pl.Datetime,
            [
            ...
            [datetime(2020, 1, 1), datetime(2020, 6, 2)],
        ),
        (
            pl.Duration,
            [
...
def test_quantile_varying_by_group_temporal_types(
    dtype: pl.DataType, test_values: list[Any], expected_values: list[Any]
) -> None:
    """Test varying quantiles with temporal data types."""
    df = pl.DataFrame(
        {
            "group": ["a", "a", "b", "b"],
            "value": test_values,
    ...
    expected = pl.DataFrame({"group": ["a", "b"], "value": expected_values})
    assert_frame_equal(result, expected)


def test_quantile_varying_by_group_edge_cases() -> None:
    """Test edge cases: out of range quantiles, single elements, empty groups."""
